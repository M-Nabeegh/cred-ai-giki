...
- `data/generated/credai-demo-v1.json` — full deterministic dataset (2000 profiles, 144,000 monthly observations) with metadata including seed, generator version, row/event counts, measured missingness, and a SHA-256 digest of the generated content. Git-ignored; regenerated by `npm run data:generate`.
- `data/generated/credai-demo-v1-features.csv` — feature matrix plus label and audit-group columns. Committed.
- `data/generated/logistic-demo-v1.1.0.json` — trained model artifact (weights, normalization stats, feature order, threshold, evaluation metrics). Committed.

## EvaluationSnapshot (`src/lib/scoring/evaluation-snapshot.ts`)
Generated file written by the training script. Carries `modelVersion`, `datasetSeed`, `generatorVersion`, dataset provenance (`datasetRowCount`, `datasetEventCount`, `datasetMissingness`, `datasetDigest`), decision `threshold`, `testRowCount`, `fairnessDiagnostics` computed from actual test predictions grouped by `synthetic_audit_group`, `consistencyChecks`, and a synthetic-only disclaimer. The customer/bank/admin UIs render these values; no static fairness literals exist in UI code.
